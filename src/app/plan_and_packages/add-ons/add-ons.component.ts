...
imageUrl: string = '';
base64Image: string = '';
metaData: string = ''; // Variable to store the selected value from the first dropdown
baseURL: string = environment.apiBASEURL;
popupTitle: string = '';
addonID: string = '';
inputValue: string = '';
capitalizedValue: string = 'Public';
...
company_id: number = 0;
total: number = 0;
doneTypingInterval = 500; // Adjust this according to your preference

// Booleans
showSpinner: boolean = false;
showFrequency: boolean = true;
showErrors: boolean = false;
showInfoBlock: boolean = false;
...
DataFoundInActiveTab: boolean = false;
DataFoundInActiveTabPagination: boolean = false;
...
moduleFeatureRows: string[] = [];
moduleFeatures: any[] = [];
moduleFeatureValues: any[] = [];
@Input('data') addon_data: string[] = [];

// Objects
valueOptions: { [key: string]: { value: string, label: string }[] } = {
  // type: [{ value: 'addons', label: 'Add-ons' }],
  public: [{value: '1', label: '1'}, {value: '0', label: '0'}]
};

// Observables
activeAddons$: Observable<any> = new Observable<any>();
inactiveAddons$: Observable<any> = new Observable<any>();
...
// ElementRef and QueryList
@ViewChild('closeDeletePopup') closeDeletePopup!: ElementRef;
@ViewChild('metadataAdd') metadataAdd!: ElementRef;
@ViewChild('billingTypeSelect') billingTypeSelect!: ElementRef;
@ViewChild('addModule') addModule!: ElementRef;
@ViewChildren('moduleSelect') moduleSelects: QueryList<ElementRef>;
@ViewChildren('deleteMetadata') deleteMetadata: QueryList<ElementRef>;
@ViewChild('priceAdd') priceAdd!: ElementRef;
@ViewChild('AddEditPopup') AddEditPopup!: ElementRef;

  constructor(private formBuilder: FormBuilder, private sanitizer: DomSanitizer, private elementRef: ElementRef,
              private appComponent: AppComponent, private breadcrumbService: BreadcrumbService,
              private httpClientRequest: HttpClientRequestService,
              private toastMessages: ToastMessagesComponent, private encryptDecrypt: EncryptDecryptService,
              private cdr: ChangeDetectorRef) {

    this.moduleSelects = new QueryList<ElementRef>();
    this.deleteMetadata = new QueryList<ElementRef>();
  ...
  fetchModuleFeatures(event: any, index: number, value: number = 0) {
    ...
    this.httpClientRequest.initiateHttpRequest(URL, body, headers, method).subscribe(
      (response) => {
        ...
        if (data[0] != undefined && data[0]['features_list'] !== undefined) {
          ...
          this.moduleFeatures.forEach((element: any, index: number) => {
          ...
          const metaDataFormArray = this.addOnsForm.get('module_features_list') as FormArray;
  ...
  fetchFeatureValue(event: any, index: number) {
    // Get the selected option element
    const selectedOption: HTMLOptionElement | null = event.target.options[event.target.selectedIndex];
    if (selectedOption) {
  ...
  SubmitAddOns() {
    ...
    if (this.addOnsForm.valid) {
      ...
      let metadata: any = {type: 'addons', public: formData.public_value};
      let prices: Price[] = formData.prices.filter((item:any) => item.unit_amount !== null && item.unit_amount !== undefined && item.unit_amount !== "")
      ...
      if (formData.metadata.length > 0) {
        ...
        formData.metadata.forEach((data: Metadata) => {
      ...
      this.httpClientRequest.initiateHttpRequest(URL, body, headers, method).subscribe(
        (response) => {
          ...
          this.addOnsForm.reset();
          (this.addOnsForm.get('module_features_list') as FormArray).clear();
          (this.addOnsForm.get('prices') as FormArray).clear();
          (this.addOnsForm.get('metadata') as FormArray).clear();
  ...
  addOnsDefaultValues() {
    ...
    const formPricesArray = this.addOnsForm.get('prices') as FormArray;
  ...
  cloneHtmlBlock() {
    ...
    let frequencies:string[] = ['month','bi-annually',"quarterly","year"];
  ...
  isErrorsObject(obj: any): obj is { required: boolean } {
  ...
  cloneHtmlBlock2() {
    ...
    // Get the prices FormArray and push the new FormGroup
    const metaDataFormArray = this.addOnsForm.get('metadata') as FormArray;
  ...
  onChangeClonedMetaData(event: any) {
  ...
  deleteModuleFeatureBlock(event: any, index: number) {
    const target = event.target;
  ...
  getAddons(flag: boolean, search: string = '') {
  ...
  serverCall(page: number, type: boolean): Observable<IServerResponse> {
    this.paginateSpinner = true;
  ...
  formatDate(dateString: any | null): string {
    if (!dateString) {
  ...
  addonToggleButton(isChecked: boolean, userId: number) {
  ...
  getSingleAddon(addon_id: string) {
    ...
    this.httpClientRequest.initiateHttpRequest(URL, body, headers, "GET").subscribe(
      (response) => {
        ...
        if (decrypted_data.product != undefined || decrypted_data.product != null || decrypted_data.product != "") {
          ...
          let updatedMetadata: { index: number, key: string, value: string }[] = [];
          ...
          let metaDataFormArray = this.addOnsForm.get('metadata') as FormArray;
          for (let i = 0; i < numberOfProperties; i++) {
          ...
          const moduleFormArray = this.addOnsForm.get('module_features_list') as FormArray;
          ...
          const priceFormArray = this.addOnsForm.get('prices') as FormArray;
          ...
          if(priceslength > 0)
          {
            for (let i = 0; i < priceslength; i++) {
              ...
              if (priceType === 'per_unit') {
                ...
                //   })
                // );
                let dbInterval:string = this.singleAddon.prices[i].recurring.interval;
                let dbIntervalCount: number = this.singleAddon.prices[i].recurring.interval_count;
            ...
            setTimeout(() => {
              let billingTypeElement = this.billingTypeSelect.nativeElement as HTMLSelectElement;
