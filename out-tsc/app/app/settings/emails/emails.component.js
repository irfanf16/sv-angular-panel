...
import { CKBox, ClassicEditor, Bold, Essentials, Italic, Mention, Paragraph, Undo, Alignment, Autoformat, BlockQuote, Heading, List, Link, CodeBlock, Table, TableToolbar, Highlight, HtmlEmbed, SourceEditing, ImageBlock, ImageUpload, ImageInsert, TableCaption, TableColumnResize, TableProperties, TableCellProperties, Superscript, Subscript, Strikethrough, SpecialCharactersText, AccessibilityHelp, AutoImage, AutoLink, Autosave, BalloonToolbar, CKBoxImageEdit, CloudServices, Code, FindAndReplace, FontBackgroundColor, FontColor, FontFamily, FontSize, HorizontalLine, ImageCaption, ImageInline, ImageInsertViaUrl, ImageResize, ImageStyle, ImageTextAlternative, ImageToolbar, Indent, IndentBlock, LinkImage, ListProperties, PageBreak, PasteFromOffice, PictureEditing, RemoveFormat, SelectAll, SpecialCharacters, SpecialCharactersArrows, SpecialCharactersCurrency, SpecialCharactersEssentials, SpecialCharactersLatin, SpecialCharactersMathematical, Underline, Template, TextTransformation, TodoList, LinkUI, ButtonLabelView } from 'ckeditor5';
let EmailsComponent = class EmailsComponent {
    constructor(appComponent, breadcrumbService, formBuilder, httpClientRequest, encryptDecrypt, sanitizer, cdr) {
    ...
    controlEmailAddEditView(flag = false) {
        this.showEmailForm = flag;
    }
    controlServicesChange(event) {
    ...
    changeValidationsAccordingToSelection(selectedValue = '') {
        let modules = ['trial', 'active_plan', 'card', 'cleanup', 'addons', 'bucket'];
    ...
    trialControlChange(event) {
        let trial_value = event.target.value.trim();
        if (trial_value) {
            const inBetween = ['before_trial_expired'];
    ...
    onEditorChange(event) {
        ...
        console.log('Editor data changed:', this.editEmailBody);
    }
    submitEmailForm() {
    ...
    getEmailDataForEdit(email_id = 0) {
    ...
    getEmails(search = '') {
        ...
        this.httpClientRequest.initiateHttpRequest(URL, body, headers, "GET").subscribe((response) => {
            ...
            if (decrypted_data.data.length > 0) {
                decrypted_data.data.forEach((email) => {
    ...
    serverCall(page) {
        ...
        return this.httpClientRequest.initiateHttpRequest(URL, body, headers, "GET").pipe(map((response) => {
            ...
            decryptedData.data.forEach((email) => {
                this.emailBody[email.id] = email.email_body;
            });
    ...
    triggerIdAssignment(emailId = 0) {
        this.emailId = emailId;
    }
    deleteFeature() {
        let URL = this.baseURL + 'api/settings/email/' + this.emailId;
        let headers = new HttpHeaders(CategoriesConfig.CATEGORY_HEADER);
        let body = this.addEmailForm.value;
        let method = "DELETE";
...
__decorate([
    ViewChild('closeBtn')
], EmailsComponent.prototype, "closeBtn", void 0);
__decorate([
    ViewChild('closeDeletePopup')
], EmailsComponent.prototype, "closeDeletePopup", void 0);
__decorate([
    ViewChild('emailPreviewModel')
], EmailsComponent.prototype, "emailPreviewModel", void 0);
